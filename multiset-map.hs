...
-}
remove elem bag = let auxRemove elem = if (elem == 1) then Nothing else Just (elem - 1) in
  Map.update auxRemove elem bag


{-
 - Busca um elemento na estrutura retornando sua quantidade. Caso o elemento nao exista, retorna 0 como a quantidade.
-}
search elem bag = Map.findWithDefault 0 elem bag
...
-}
minus bag1 bag2 = let auxMinus elem1 elem2 = if ((elem1 - elem2) > 0) then Just (elem1 - elem2) else Nothing in
  Map.differenceWith auxMinus bag1 bag2
...
-}
sum2 bag1 bag2 = Map.unionWith (+) bag1 bag2
